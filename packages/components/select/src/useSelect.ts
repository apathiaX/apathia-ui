...
import { onClickOutside, useResizeObserver } from '@apathia/shared'
import {
  SelectValueType,
  Direction,
  SelectOption,
...
export default function useSelect(
  ...
  const tempMap = shallowReactive<Record<string, SelectOption>>({})
  ...
  const selectState = reactive({
    ...
    filterStr: '',
    optionIds: [] as number[],
    optionMap: {} as Record<string, SelectOption>,
    focusId: 0,
    pendingFocusId: 0,
    innerChange: false,
    indeed: true,
  ...
  const getInputProps = (): InputHTMLAttributes => ({
    type: 'text',
    tabindex: '0',

...
const _sfc_main = defineComponent({
  ...
  setup(props, ctx) {
    ...
    const disableInput = useInjectProp("FormDisabled", false, disabled);
    const userProps = {
      modelValue,
      trueValue,
      falseValue,
    ...
    const styles = {
      ...
      inner: tw(style`relative block w-4 h-4 top-0 left-0 bg-white border border-gray-500 rounded outline-none`, css`
          transition: all 0.3s;
          &::after {
            content: '';
            display: table;
      ...
      `),
      disabledInner: tw(style`bg-gray-100 border-gray-500 cursor-not-allowed outline-none`, css`
          &::after {
